...
public class RecipeDAOTest {
    ...
    public static void main(String[] args) {
        ...
        testCreate2();
        testGetAll2(); // 全レコード確認
        System.out.println("recipe_id=1のレコード更新");
        testUpdate();
        testGetAll2(); // 全レコード確認
        System.out.println("recipe_id=1のレコード削除");
        testRemove();
        testGetAll2(); // 全レコード確認
        ...
        if (result != null) {
            for (Map.Entry<Integer, Recipe> entry : result.entrySet()) {
                ...
                System.out.println("recipe_content:"
                        + (recipe.getRecipeContent() == null || recipe.getRecipeContent().length() < 2 ? null
                                : recipe.getRecipeContent().substring(0, 2)));
            }
        } else {
            System.out.println("testGetAll:RECIPEテーブルレコード無し");
        }
    ...
    public static void testUpdate() {
        ...
        if (result) {
            System.out.println("recipe_id=1のレコード更新成功");
        } else {
            System.out.println("recipe_id=1のレコード更新失敗");
        }
    }
    ...
    public static void testRemove() {
        ...
        if (result) {
            System.out.println("recipe_id=2のレコード削除成功");
        } else {
            System.out.println("recipe_id=2のレコード削除失敗");
        }
    }
